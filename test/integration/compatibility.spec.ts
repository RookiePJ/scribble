...
function compareVars(
    ...
    compilerVersion: ABIEncoderVersion
): boolean {
    ...
    const bSig =
        b instanceof VariableDeclaration
            ? b.getterCanonicalSignature(compilerVersion)
            : b.canonicalSignature(compilerVersion);
    return a.getterCanonicalSignature(compilerVersion) === bSig;
}
...
function findCorrespondigFn(
    ...
    for (const member of members) {
        if (
            ...
            fn.name === member.name &&
            fn.kind === member.kind &&
            fn.canonicalSignature(compilerVersion) === member.canonicalSignature(compilerVersion)
        ) {
            return member;
        }
    }
...
function checkCompatibility(
    ...
    for (const memberA of membersA) {
        if (memberA instanceof VariableDeclaration) {
            ...
            if (!compareVars(memberA, memberB, compilerVersion)) {
                throw new Error(
            ...
            const memberB = findCorrespondigFn(memberA, membersB, compilerVersion);
...
describe("Interface compatibility test", () => {
    ...
    for (const sample of samples) {
        describe(`Sample ${sample}`, () => {
            ...
            let abiVersion: ABIEncoderVersion;
            ...
            before(() => {
                ...
                inAst = result.units;
                abiVersion = getABIEncoderVersion(inAst, compilerVersion);
            });

            const compareSourceUnits = (
                inAst: SourceUnit[],
                outAst: SourceUnit[],
                ...
                for (const [name, inContract] of input.entries()) {
                    ...
                    checkCompatibility(inContract, outContract, abiVersion);
                }
            };
            ...
            it("Instrumented source in 'log' mode has compatible external interface", () => {
                ...
                compareSourceUnits(inAst, result.units, abiVersion);
            });
